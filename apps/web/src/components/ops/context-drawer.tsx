    ...
    return (
      <aside className="glass-panel rounded-[28px] border border-white/10 p-3 shadow-panel">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <SeverityPill alert={event} compact />
        </div>
        <p className="mt-3 text-sm leading-6 text-slate-300">{event.short_message}</p>
        ...
        <div className="mt-3 rounded-2xl border border-cyan-300/15 bg-cyan-300/7 p-3">
          <p className="text-[11px] uppercase tracking-[0.2em] text-cyan-200">Narrativa</p>
          <p className="mt-2 text-sm leading-6 text-slate-200">{event.narrative_summary}</p>
        </div>

        <div className="mt-3 flex items-center gap-2">
          <Link
            href={`/incident/${event.id}`}
            className="inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-white px-4 py-3 text-sm font-medium text-slate-900"
          >
            Ver detalle
